...
class MercadoLibreAutentication(http.Controller):

    @http.route(['/mercadolibre_log_in'], type='http', auth="user", methods=['GET'], website=True)
    def meli_login(self, **kwargs):
        configuration = request.env['mercadolibre.configuration'].get_config_with_mercadolibre_current_login()
        meli = configuration._get_mercadolibre_service()
        redirect_uri = configuration.mercadolibre_redirect_uri
        if not kwargs:
            kwargs = {}
        if kwargs.get('error'):
